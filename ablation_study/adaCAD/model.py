...
from torch_geometric.utils import degree, sort_edge_index, softmax
...
#         # self.gatconv = GATConv(emb_dim, emb_dim, heads=3)
...
class AdaCAD(MessagePassing):
    ...
    def compute_transP(self, cd, edge_index):
        ...
        transP = softmax(pipj, row, num_nodes=cd.size(0))   # 与F.softmax(A*(cd@cd.T), dim=-1)不一样, 因为F.softmax会给0也分配权重
    ...
    def message(self, x_j, transP): # x_j: x[edge_index[0]]
    ...
    def __init__(self, params):
        ...
        for _ in range(self.num_layer):
            if self.gnn_type == 'gin':
                ...
                # self.convs.append(GraphAttentionConv(self.edge_dim,self.emb_dim))
                self.convs.append(GATConv(self.emb_dim, self.emb_dim))
            elif self.gnn_type == 'adaCAD':
                self.convs.append(AdaCAD(params['Kstep_for_AdaCAD'], params['beta_for_AdaCAD'], params['drop_ratio']))
            else:
...
class predictModel(torch.nn.Module):
    ...
    def forward(self, type, value, edge_index,edge_attr,batch):
        ### feature embedding 
        type_embs = self.type_encoder(type)
        value_embs = self.value_encoder(value)
        h = torch.cat((type_embs,value_embs),dim=1)
...
# class GCN_wo_edgeattr(torch.nn.Module):
#     def __init__(self, params):
#         super(GCN_wo_edgeattr, self).__init__()
#         self.emb_dim = params["emb_dim"]
#         self.type_nums = params["type_nums"]
#         self.value_nums =params["value_nums"]
#         self.type_encoder = torch.nn.Embedding(self.type_nums, self.emb_dim)
...
#         nfeat = self.emb_dim*2
#         for nhid, act in zip(params['nhids'], params['acts']):
#             # self.layers.append(pyg.nn.GCNConv(nfeat, nhid, cached=False, 
#             #     add_self_loops=True, normalize=True, bias=params['bias']).to(self.device))
#             self.layers.append(pyg.nn.GCNConv(nfeat, nhid, cached=False, 
#                 add_self_loops=True, normalize=True, bias=params['bias']))
#             self.layers.append(getattr(torch.nn, utils._act_dict.get(act, None))())
#             self.layers.append(torch.nn.Dropout(params['dropout']))
#             nfeat = nhid
#         # self.layers.append(pyg.nn.GCNConv(nfeat, 2, cached=False, add_self_loops=True, normalize=True, bias=params['bias']).to(self.device))
#         self.layers.append(pyg.nn.GCNConv(nfeat, self.emb_dim*2, cached=False, add_self_loops=True, normalize=True, bias=params['bias']))
...
#             if isinstance(layer, pyg.nn.GCNConv):
