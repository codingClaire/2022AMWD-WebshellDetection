...
from utils import bestGPU, load_pickle,setup_seed,save_csv_with_configname
import copy
...
def main(net_parameters):
    ...
    train_dataset_dir = f"split{net_parameters['split_type']}_{net_parameters['undirected']}/train_dataset/"
    val_dataset_dir = f"split{net_parameters['split_type']}_{net_parameters['undirected']}/val_dataset/"
    test_dataset_dir = f"split{net_parameters['split_type']}_{net_parameters['undirected']}/test_dataset/"
    ...
    if net_parameters["extract"] == True and not os.path.exists(train_dataset_dir):
        ...
        extract_and_save_graph_data(net_parameters['undirected'], 1,train_num+1, train_dir,train_dataset_dir,type_word2id,value_word2id)
        extract_and_save_graph_data(net_parameters['undirected'], train_num+1,train_num+val_num+ 1, train_dir,val_dataset_dir,type_word2id,value_word2id)
        extract_and_save_graph_data(net_parameters['undirected'], train_num+val_num+ 1,total_num+1, train_dir,test_dataset_dir,type_word2id,value_word2id)
    else: 
        type_word2id  = load_pickle(vocab_dir + "word2id.node_types.pkl")
        value_word2id = load_pickle(vocab_dir + "word2id.node_values.pkl")
    ...
    for epoch in range(net_parameters["load"] + 1, net_parameters["epochs"] + 1):
        ...
        model.train()
        losses = []
        for _, batch in enumerate(tqdm(train_loader,desc="Iteration",mininterval=30)):
            batch = batch.to(device)
            pred = model(batch.node_type,batch.node_value,batch.edge_index,batch.edge_attr,batch.batch)
            optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    if isinstance(total_parameters["seed"], list):
        ...
        fout.write(f"{args.config.split('/')[-1][:-5]},")
